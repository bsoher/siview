...
class MultiDirDialog(wx.Dialog):
    ...
    def GetPaths(self):
        ...
        # user has made. 
        #
        # NB. bjs - on windows I was getting odd paths that included the string
        # name of the drive (e.g. u'Data (D:)\\temp') because of the code that
        # parses the text in the TreeCtrl. But, as of wx 2.9+ we can use the 
        # overloaded GetPath(ItemId) method to get that path for a given item 
        # in the tree. And this seems to return the drive letter only.

        folders = []
        treeCtrl = self.dirCtrl.GetTreeCtrl()
        selections = treeCtrl.GetSelections()
        for item in selections:
            folders.append(self.dirCtrl.GetPath(item))
