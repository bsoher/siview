...
BaseNotebook subclasses wx.Notebook and 
BaseAuiNotebook subclasses (surprise!) wx.AuiNotebook.

Both classes add the same thing, which are the properties and
...
class _NotebookHelper(object):
    ...
    def show_welcome_tab(self):
        ...
        html_control = wx.html.HtmlWindow(self, -1, wx.DefaultPosition, wx.Size(400, 300))
        html_control.SetPage(self.WELCOME_TAB_TEXT)

        # Passing an image to AddPage breaks under 2.9.
        # ref: http://permalink.gmane.org/gmane.comp.python.wxpython/91454
        self.AddPage(html_control, self.WELCOME_TAB_TITLE, False)



class BaseAuiNotebook(aui.AuiNotebook, _NotebookHelper):
...
class BaseNotebook(wx.Notebook, _NotebookHelper):
