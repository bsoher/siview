...
# Note that this code can't assume too much about what's installed, nor about SIView, so
...
def check_dependencies():
    ...
    Packages are in this list regardless of whether or not SIView needs them.
...
def report_dependencies(to_stdout=True):
    """Creates a report of the status of SIView's dependencies. Failures are explicit about what's
    ...
    if missing or insufficient:
        report.append("SIView might not run properly because some dependencies are unmet.")
        ...
        if insufficient:
            report.append(
                "The following dependencies are installed, but SIView needs a different version:")
            for requirement in insufficient:
                installed_package = installed[requirement.name]
                specifiers = ';'.join([str(specifier) for specifier in requirement.specifier])
                requirement = "%s%s" % (requirement.name, specifiers)

                report.append(" * %s is installed; SIView requires %s" % \
                              (installed_package, requirement))

        report.append('')
        report.append("One can find information about satisfying these dependencies here:")
        report.append("https://xxx.github.io/xx.io/")
    else:
        report.append('')
        report.append("Good news -- all of SIView's dependencies are installed!")
