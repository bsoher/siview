...
from siview.analysis.constants import VoigtDefaultFixedT2
from siview.analysis.constants import VoigtDynMetabolite

from wx.lib.agw.floatspin import FloatSpin, EVT_FLOATSPIN, FS_LEFT, FS_RIGHT, FS_CENTRE, FS_READONLY
from siview.common.wx_gravy.widgets.floatspin_multiplier.floatspin_multiplier_base import FloatSpinMultiplier




class DynamicListVoigtMetabolite(object):
    ...
    def _add_row(self, full_name, flag, mult, pkppm, wid, dbflag, t2val, ph0val):
        ...
        scale.multiplier = VoigtDynMetabolite.AREA_SCALE_MULT
        scale.SetDigits(VoigtDynMetabolite.AREA_SCALE_DIGITS)
        scale.SetIncrement(VoigtDynMetabolite.AREA_SCALE_INCR)
        scale.SetRange(VoigtDynMetabolite.AREA_SCALE_MIN,VoigtDynMetabolite.AREA_SCALE_MAX)
        scale.SetValue(mult)
        scale.SetMinSize(floatspin_size)
        
        shift.SetDigits(VoigtDynMetabolite.SEARCH_CENTER_DIGITS)
        shift.SetIncrement(VoigtDynMetabolite.SEARCH_CENTER_INCR)
        shift.SetRange(minppm,maxppm)
        shift.SetValue(pkppm)
        shift.SetMinSize(floatspin_size) 

        width.SetDigits(VoigtDynMetabolite.SEARCH_WIDTH_DIGITS)
        width.SetIncrement(VoigtDynMetabolite.SEARCH_WIDTH_INCR)
        width.SetRange(VoigtDynMetabolite.SEARCH_WIDTH_MIN,VoigtDynMetabolite.SEARCH_WIDTH_MAX)
        width.SetValue(wid)
        width.SetMinSize(floatspin_size) 

        dbppm.SetValue(dbflag)

        fixt2.SetDigits(VoigtDefaultFixedT2.DIGITS)
        fixt2.SetIncrement(VoigtDefaultFixedT2.INCR)
        fixt2.SetRange(VoigtDefaultFixedT2.MIN,VoigtDefaultFixedT2.MAX)
        fixt2.SetValue(t2val)
        fixt2.SetMinSize(floatspin_size) 

        phas0.SetDigits(VoigtDynMetabolite.SEARCH_PHASE0_DIGITS)
        phas0.SetIncrement(VoigtDynMetabolite.SEARCH_PHASE0_INCR)
        phas0.SetRange(VoigtDynMetabolite.SEARCH_PHASE0_MIN,VoigtDynMetabolite.SEARCH_PHASE0_MAX)
