...
class ChainSpatialCrt(Chain):
    """
    Building block object used to create a processing chain for MRSI data.

    Processes: SI coil-combination, individual voxel ECC from metabolite 
    cycled water data, individual voxel frequency correction.  All this is 
    done for each individual data set (by reps and mcycle reps). They have 
    ...
    def run(self, voxels, entry='all'):
        ...
        self.data = self._dataset.get_source_data('grid')
