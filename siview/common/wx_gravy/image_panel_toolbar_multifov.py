...
class ImagePanelToolbarMultiFov(wx.Panel):
    """
    ...
    def __init__(self, parent, naxes=1, 
        ...
        self.toolbar = NavToolbarMultiFov(self.canvas,
                                            self, 
                                            vertOn=vertOn, 
                                            horizOn=horizOn,
                                            lcolor=lcolor,
...
class _Mode(str, Enum):
    """ Repeated here to add LEVEL mode """
    NONE = ""
    PAN = "pan/zoom"
    ZOOM = "zoom rect"
    LEVEL = 'width/level'
...
class NavToolbarMultiFov(NavigationToolbar2, wx.ToolBar):
    ...
    def __init__(self, canvas, parent, vertOn=False, horizOn=False, lcolor='gold', lw=0.5, coordinates=True):
        ...
        self.mode = _Mode.NONE  # repeat here to ensure local _Mode class
    ...
    def local_setup_before_event(self, ):
        if self._idRelease is not None:
            self._idRelease = self.canvas.mpl_disconnect(self._idRelease)
        if self._idPress is not None:
            self._idPress   = self.canvas.mpl_disconnect(self._idPress)

    def local_setup_after_event(self, ):
